...
try {
    ...
    $inputs['voice_notes'] = $_POST['voice_notes'] ?? [];
    $inputs['poll_options'] = $options ?? null;
    $inputs['album'] = $_POST['album'] ?? '';
    $inputs['video'] = $_POST['video'] ?? null;
    $inputs['audio'] = $_POST['audio'] ?? null;
    $inputs['file'] = $_POST['file'] ?? null;
    $inputs['location'] = $_POST['location'] ?? null;
    $inputs['feeling_action'] = $_POST['feeling_action'] ?? null;
    $inputs['colored_pattern'] = $_POST['colored_pattern'] ?? null;

    // Add additional fields
    $inputs['is_pushed'] = isset($_POST['is_pushed']) ? $_POST['is_pushed'] : '0'; // Default to 0 if not set
    $inputs['is_pinner_approve'] = isset($_POST['is_pinner_approve']) ? $_POST['is_pinner_approve'] : '0'; // Default to 0 if not set

    // Publish post and save to database
    $post = $user->publisher($inputs);

    // Save push record
    $db->query("INSERT INTO push_records (post_id, user_id, is_pushed, is_pinner_approve) VALUES (?, ?, ?, ?)", [
        $post['post_id'], // Assuming the post ID is returned from the publisher method
        $user->_data['user_id'], // Current user's ID
        $inputs['is_pushed'],
        $inputs['is_pinner_approve']
    ]);

    // Check if the post is approved for feed visibility
    if ($inputs['is_pinner_approve'] == '1') {
        // assign variables
        ...
        // If not approved, you can choose to return a message or just skip sending to feed
        return_json(['success' => false, 'message' => __('Post not approved for feed visibility.')]);
    }
