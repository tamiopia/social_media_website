...
api['pushpin/request'] = ajax_path + "posts/requests.php";
...
$(function () {
  ...
  // run publisher
  /* publisher focus */
  $('body').on('click','.js_publish_toggler_bar', function (e) {
  ...
  $('body').on('click', '.js_publisher', function () {
    ...
    /* NEW CODE: Check if Push Pin is toggled on */
    var push_pin_enabled = publisher.find('.js_push_pin_toggle').is(":checked");
    var push_pin_message = '';
    var push_pin_user_id = '';
    
    if (push_pin_enabled) {
        /* If push pin is enabled, get the push pin message and selected user */
        push_pin_message = publisher.find('textarea[name="push_pin_message"]').val();
        push_pin_user_id = publisher.find('#search_user').data('selected-user-id');  // Assuming the search input stores user ID as a data attribute
    }
    ...
    /* Submit post data via AJAX */
    console.log(`push_pin_message`, push_pin_message)
    console.log(`push_pin_user_id`, push_pin_user_id)
    ...
    $.post(api['posts/post'], {
        ...
        /* Include push pin data */
        'push_pin_enabled': push_pin_enabled,
        'push_pin_message': push_pin_message,
        'push_pin_user_id': push_pin_user_id
    }, function (response) {
        if (response.callback) {
            /* button reset */
            ...
            button_status(_this, "reset");
            /* other post publishing reset and UI updates */
