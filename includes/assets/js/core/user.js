...
$(function () {
  ...
  $('body').on('focus', '.js_mention', function () {
    var input = $(this);
    input.triggeredAutocomplete({
      hidden: '#hidden_inputbox',
      source: function (request, response) {
        $.ajax({
          url: api['users/mention'], // Fetch user data from the API
          data: { term: request.term }, // Send the user's input as a parameter
          success: function (data) {
            console.log(data); // Log the user data returned by the API
